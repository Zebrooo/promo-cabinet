...
[B. S3 bucket]  catalogue.json  =  ORDERED Promo[]  (array index = queue position)
        ▲ GetObject (read-only)
        │
[C. abhPromo backend]  config-service reads the ordered catalogue from S3
        │  POST /models → select-promo → walk queue top→bottom, return FIRST eligible
        ▼
[D. existing chain, NOT built]  BFF reads per-user show data from DataSync,
        calls abhPromo with impressionCounts + lastShownAt → renders via promo-renderer
```
...
9. **abhPromo stays stateless & reads fresh per request, no cache.** Per-user show data
   (`impressionCounts`, `lastShownAt`) is supplied by the **BFF**, which sources it from a
   per-user **DataSync** key/value store (`promoId → timestamp`). DataSync and the BFF are
   out of scope.
...
feature inside the cabinet; the DataSync store and the BFF; changing `promo-renderer`.
...
### Selection logic (`promo-selector`)
...
- New **`CooldownChecker`**: eligible iff `cooldownHours === 0` OR there is no
  `lastShownAt[promo.id]` OR `now - lastShownAt[promo.id] >= cooldownHours`.
- Checker chain (lazy `.every()`): `DateChecker → UserChecker (targeting) →
  LimitChecker (maxImpressionsPerUser) → CooldownChecker`.
- Selector walks the catalogue **in array order** and returns the **first** promo that
  passes all checkers (no more max-by-score). Returns the chosen `Advertisement`, or a
  skipped/empty result if none qualify.
