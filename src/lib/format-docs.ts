...
const FORMAT_NOTES: Partial<Record<PromoFormat, string[]>> = {
  promoline: [
    'Контент совпадает с `inline` плюс единственное своё поле `afterListings` (позиция в ленте) — витрина рендерит promoline тем же inline-рендерером (`@zebrooo/promo-renderer` формата `promoline` не знает, кабинет мапит его на `inline` в `toAdvertisement`).',
