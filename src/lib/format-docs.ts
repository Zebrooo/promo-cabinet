...
export const FIELD_DESCRIPTIONS: Record<string, string> = {
  ...
  'targeting.advertiser': 'Ось «Рекламодатель»: условия по рекламным кампаниям зрителя (ad_campaigns витрины) и мастеру подачи РК (только залогиненные — анонимам не показывается; условия по И; пустой блок = гейта нет)',
  'targeting.advertiser.campaignStatuses': 'Есть РК хотя бы в одном из статусов сейчас (слаги ad_campaigns.status, до 10, внутри списка — ИЛИ; известные: pending, active)',
  'targeting.advertiser.hasActiveCampaign': 'true = есть РК в статусе active сейчас; false = ни одной активной',
  'targeting.advertiser.everLaunched': 'true = хоть раз запускал РК (кампания доходила до active); false = никогда не запускал',
  'targeting.advertiser.launchedWithinDays': 'Только при everLaunched=true: последняя запущенная РК не старше N дней (1–365; пусто = за всё время)',
  'targeting.advertiser.abandonedWizard': 'true = открывал мастер подачи РК и не отправил (form_start без form_submit_success за окно); false = брошенного мастера нет',
  'targeting.advertiser.wizardLookbackDays': 'Только при abandonedWizard=true: окно поиска брошенного мастера, дней (1–90; пусто = дефолт BFF 30)',
  ...
  cooldownHours: 'УСТАРЕВШЕЕ: часы; BFF читает только без новых полей пауз — как паузу формата N×60 минут плюс правило «не повторять себя» N×60',
  cooldownSelfMinutes: 'Общая пауза формата: после показа этого промо другие промо того же формата на том же устройстве не показываются N минут (само промо не ждёт; 0/пусто = нет)',
  cooldownPromos: 'Направленные паузы: не показывать это промо N минут после показа указанных промо; ссылка на себя = «не повторять чаще N минут» (до 20 правил)',
  'cooldownPromos[].promoId': 'id промо, после показа которого действует пауза (можно своё)',
  'cooldownPromos[].minutes': 'Длина паузы, целых минут (1 … 525 600)',
  ...
  afterListings: 'Promoline: через сколько органических карточек ленты каталога стоит строка (4–50; пусто = умолчание витрины — после четвёртой)',
};
...
const FORMAT_NOTES: Partial<Record<PromoFormat, string[]>> = {
  promoline: [
    'Контент совпадает с `inline` плюс единственное своё поле `afterListings` (позиция в ленте). Нативный формат `@zebrooo/promo-renderer`: та же вёрстка, что у inline, но `data-format="promoline"` и класс `zr-promoline`; события аналитики идут с format=promoline.',
    'Поверхность: строка между объявлениями в ленте авто/шин/дисков — после N-й органической карточки (`afterListings`, по умолчанию четвёртой), один показ на документ, участвует в frequency-капах.',
    'Правило «ниже видимой области»: витрина выбирает промо, когда четвёртая карточка уже в зоне первого экрана, и вставляет строку только ниже видимой области (без сдвига вёрстки) — отсюда нижняя граница `afterListings` = 4. Если карточек в ленте меньше, чем задано, строка встанет после последней.',
  ],
