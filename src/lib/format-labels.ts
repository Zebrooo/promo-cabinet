 ...
 *  имя типа (контракт: Inline, Promoline, Topline, Popup, Fullscreen, DivKit,
 *  Tooltip, Multistep — сверено с promoFormatSchema); `sub` — русское пояснение.
...
export const FORMAT_LABEL: Record<Promo['format'], { name: string; sub: string }> = {
  ...
  promoline:  { name: 'Promoline',  sub: 'Строка в ленте каталога' },
