...
describe('advertiserTargetingSchema (ось «Рекламодатель», контракт AdvertiserChecker BFF)', () => {
  ...
  it('strips the retired endsWithinDays key (у ad_campaigns нет даты окончания; старый пул не должен ломаться)', () => {
    const res = advertiserTargetingSchema.safeParse({ everLaunched: true, endsWithinDays: 7 });
    expect(res.success).toBe(true);
    if (res.success) expect(res.data).toEqual({ everLaunched: true });
  });

  it('rejects negative / fractional money', () => {
  ...
  it('rejects money conditions that contradict launch state', () => {
