...
function allowedFormatsFor(target: NonNullable<Promo['deviceTarget']>): readonly PromoFormat[] {
  ...
  // 'promoline' — строка между объявлениями в ленте каталога: витрина рендерит
  // её inline-рендерером, а самого формата пакет не знает (и, скорее всего, не
  // узнает) — поэтому он всегда добавляется локально. Ограничений по
  // устройствам нет: лента каталога одинакова на десктопе и на тач.
  // Set-дедуп защищает от задвоения тайла при любой версии пакета.
  return [...new Set<PromoFormat>([...base, 'multistep', 'custom', 'promoline'])];
}
