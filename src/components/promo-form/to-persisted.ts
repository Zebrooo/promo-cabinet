...
function normalize(rawValues: Promo): Promo {
  ...
  // Под-блоки targeting.* без настоящего критерия и «чужие» пороги профиля
  // визита вычищает общая с пуш-рассылками нормализация (lib/targeting-normalize.ts).
  const targeting = normalizeTargeting(values.targeting);
